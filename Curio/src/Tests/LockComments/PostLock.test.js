...
//     // Wait for FetchPostLockStatus to be called
//     await waitFor(() => expect(FetchPostLockStatus).toHaveBeenCalled());
    
//     expect(FetchPostLockStatus).toHaveBeenCalledWith("1");
...
//   });
  ...
  it('renders the lock button', () => {
    const { getByTestId } = render(<PostLock id="1" />);
